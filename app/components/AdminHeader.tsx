                  ...
                  <View className="flex-1">
                    ...
                    <Text className="text-slate-400 text-sm">
                      {result.type === "customer" &&
                        `${result.email} ΓÇó ${result.status}`}
                      {result.type === "technician" &&
                        `${result.techId} ΓÇó ${result.status} ΓÇó Rating: ${result.rating}`}
                      {result.type === "partner" &&
                        `${result.domain} ΓÇó ${result.plan} plan ΓÇó ${result.status}`}
                      {result.type === "emergency" &&
                        `${result.customer} ΓÇó ${result.location} ΓÇó ${result.status}`}
                    </Text>
                  </View>

                  <View className="w-6 h-6 bg-white/10 rounded-full items-center justify-center">
                    <Text className="text-slate-400 text-xs">ΓåÆ</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </ScrollView>
