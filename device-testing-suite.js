...
console.log('📱 Device Testing Suite for Mobile Responsiveness');
console.log('=================================================\n');
...
function validateResponsiveDesign(device) {
  console.log(`📱 Testing ${device.name} (${device.screen.width}x${device.screen.height})`);
  ...
  console.log(`  📊 Device compatibility score: ${score}%\n`);
...
function validateButtonFunctionality() {
  console.log('🔘 Button Functionality Validation');
  console.log('==================================\n');
  ...
  dashboardComponents.forEach(component => {
    ...
    console.log(`  ⚡ Functional patterns: ${functionalPatterns} (${Math.round(functionalityRatio * 100)}%)`);
    console.log(`  ♿ Accessibility: ${accessibilityLabels} (${Math.round(accessibilityRatio * 100)}%)`);
    console.log(`  👆 Touch targets: ${touchTargetStyles} (${Math.round(touchTargetRatio * 100)}%)`);
  ...
  console.log('📊 Button Functionality Summary:');
  console.log('================================');
...
console.log(`  ✅ Compatible devices: ${passedDevices}/${deviceResults.length}`);
...
console.log(`  🎯 High priority devices: ${highPriorityPassed}/${highPriorityDevices.length} passed`);

// Button functionality summary
console.log('\n🔘 Button Functionality:');
console.log(`  ⚡ Functionality: ${Math.round(buttonResults.functionalityRatio * 100)}%`);
console.log(`  ♿ Accessibility: ${Math.round(buttonResults.accessibilityRatio * 100)}%`);
console.log(`  👆 Touch targets: ${Math.round(buttonResults.touchTargetRatio * 100)}%`);
...
console.log(`📊 ${Math.round(overallScore)}% - ${overallScore >= 90 ? 'Excellent' : overallScore >= 80 ? 'Good' : overallScore >= 70 ? 'Fair' : 'Needs Improvement'}`);

console.log('\n📋 Recommendations:');
console.log('===================');

if (buttonResults.accessibilityRatio < 0.8) {
  console.log('⚠️  Improve accessibility: Add accessibilityRole and accessibilityLabel to more buttons');
}

if (buttonResults.touchTargetRatio < 0.8) {
  console.log('⚠️  Improve touch targets: Ensure all buttons meet 44px minimum size');
}

if (passedDevices < deviceResults.length) {
  console.log('⚠️  Device compatibility: Test and optimize for failing devices');
}

if (buttonResults.functionalityRatio < 0.8) {
  console.log('⚠️  Button functionality: Ensure all buttons have proper onPress handlers');
}
